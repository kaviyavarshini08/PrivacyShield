...
def generate_excel_report(test_results, output_path):
    """
    Builds a styled Excel Workbook summarizing Appium mobile test runs.
    """
    ...
    ws_summary["A1"] = "PrivacyShield - E2E Mobile Appium Analytics Report"
    ...
    # Metric table title
    ws_summary["A3"] = "E2E Mobile Test Execution Metrics"
    ...
    headers = ["Test ID", "Test Category", "Component/Area", "Appium E2E Verification Checkpoint", "Status", "Duration (s)"]
    ...
    for row_num, result in enumerate(test_results, 2):
        ...
        ws_details.cell(row=row_num, column=2, value=result.get("test_type", "E2E Mobile Testing")).font = standard_font
